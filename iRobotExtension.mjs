...
//import disconnectedIcon from './disconnected.png';
...
var ExtensionBlocks = /*#__PURE__*/function () {
  function ExtensionBlocks(runtime) {
    ...
    // Scratch Linkへの接続を試みる
    this._setupScratchLink();
  }

  /**
   * Scratch Linkとの接続をセットアップ
   */
  return _createClass(ExtensionBlocks, [{
    key: "_setupScratchLink",
    value: function _setupScratchLink() {
      var _this = this;
      this._socket = new WebSocket('ws://127.0.0.1:20110/scratch/ble');
      this._socket.onopen = function () {
        log$1.log('Scratch Linkに接続しました');
      };
      this._socket.onclose = function () {
        log$1.log('Scratch Linkから切断されました');
        _this._isConnected = false;
        _this.runtime.emit('EXTENSION_STATUS_CHANGED', {
    ...
    value: function _handleMessage(message) {
      switch (message.type) {
        case 'connect':
          ...
          this._peripheral = message.peripheralId;
          this.runtime.emit('EXTENSION_STATUS_CHANGED', {
            extensionId: EXTENSION_ID,
            status: {
              status: 1,
              msg: 'connected'
        ...
        case 'characteristicValue':
          this._handleNotifications(message.message);
          break;
      }
    }
    ...
    key: "_handleNotifications",
    value: function _handleNotifications(data) {
    ...
    value: function connectBLE(args, callback) {
      if (!this._socket) {
        log$1.error("Scratch Linkに接続されていません");
        callback();
        return;
      }
      var request = {
    ...
    // 他のコマンド送信メソッド (driveForward, driveBackward, etc.) も
    // Scratch Link形式に変更する必要があります
  }, {
    key: "driveForward",
    value: function driveForward(args) {
      ...
      var request = {
        type: 'write',
        characteristic: TX_CHARACTERISTIC,
    ...
    value: function getInfo() {
      ...
      return {
        ...
        status: {
          status: this._isConnected ? 1 : 2,
          msg: this._isConnected ? 'connected' : 'disconnected'
        },
        blocks: [{
          opcode: 'connectBLE',
          blockType: BlockType$1.COMMAND,
          text: 'BLE UARTデバイスに接続する',
          func: 'connectBLE'
        }
        // 他のブロック定義...
        ]
