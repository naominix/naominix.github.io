import formatMessage$1 from 'format-message';
...
var en = {
	"iRobotExtension.entry.name": "iRobotExtension",
	"iRobotExtension.entry.description": "Description of this extension"
};
var ja = {
	"iRobotExtension.entry.name": "iRobotExtension",
	"iRobotExtension.entry.description": "拡張機能の説明"
};
var translations = {
	en: en,
	ja: ja,
	"ja-Hira": {
	"iRobotExtension.entry.name": "iRobotExtension",
	"iRobotExtension.entry.description": "かくちょうきのうのせつめい"
}
};
...
var entry = {
  get name() {
    return formatMessage({
      id: 'iRobotExtension.entry.name',
      defaultMessage: 'iRobotExtension',
      description: 'name of the extension'
    });
  },
  extensionId: 'iRobotExtension',
  extensionURL: 'https://naominix.github.io/dist/iRobotExtension.mjs',
  ...
  get description() {
    return formatMessage({
      ...
      id: 'iRobotExtension.entry.description'
    });
  },
  featured: true,
  disabled: false,
...
var ble = navigator.bluetooth ? WebBLE : BLE;
...
var RootDevice = /*#__PURE__*/function () {
  ...
  return _createClass$1(RootDevice, [{
    ...
    /**
     * Scan for devices using WebBluetooth or BLE
    ...
    value: function scan() {
      ...
      // Try to use Scratch BLE if available
      if (BLE$1.isAvailable()) {
        return this._connectWithBLE();
      }

      // Otherwise use WebBluetooth
      if (navigator.bluetooth) {
        return this._connectWithWebBluetooth();
      }
      this._deviceState = DEVICE_STATE.DISCONNECTED;
    ...
    /**
     * Connect to a device using Scratch BLE
     * @returns {Promise} - resolves when connected
     */
  }, {
    key: "_connectWithBLE",
    value: function _connectWithBLE() {
      var _this = this;
      this._ble = new BLE$1(this._runtime, this._extensionId);
      return this._ble.requestDevice({
        filters: [{
          services: [ROOT_SERVICE_UUID]
        }]
      }).then(function (deviceInfo) {
        _this._device = deviceInfo;
        return _this._ble.connectDevice(deviceInfo);
      }).then(function () {
        _this._deviceState = DEVICE_STATE.CONNECTED;
        return _this._setupBLEServices();
      }).catch(function (err) {
        _this._deviceState = DEVICE_STATE.DISCONNECTED;
        log$2.warn("Error connecting to device via BLE: ".concat(err.message));
        return Promise.reject(err);
      });
    }

    /**
     * Set up services and characteristics for Scratch BLE
     * @returns {Promise} - resolves when services and characteristics are set up
     */
  }, {
    key: "_setupBLEServices",
    value: function _setupBLEServices() {
      var _this2 = this;
      return this._ble.readCharacteristic({
        serviceId: ROOT_SERVICE_UUID,
        characteristicId: ROOT_UART_TX_CHARACTERISTIC_UUID,
        startNotifications: true
      }).then(function (value) {
        _this2._ble.addCharacteristicCallback({
          serviceId: ROOT_SERVICE_UUID,
          characteristicId: ROOT_UART_TX_CHARACTERISTIC_UUID,
          callback: _this2._onMessage
        });

        // Set up disconnect callback
        _this2._ble.addDisconnectCallback(_this2._onDisconnect);
    ...
    /**
     * Connect to a device using WebBluetooth
     * @returns {Promise} - resolves when connected
     */
  }, {
    key: "_connectWithWebBluetooth",
    value: function _connectWithWebBluetooth() {
      var _this3 = this;
      return navigator.bluetooth.requestDevice({
        ...
        _this3._device = device;
        device.addEventListener('gattserverdisconnected', _this3._onDisconnect);
        ...
        log$2.warn("Error connecting to device via WebBluetooth: ".concat(err.message));
        return Promise.reject(err);
      });
    }
    ...
    value: function _onDisconnect() {
      this._deviceState = DEVICE_STATE.DISCONNECTED;
      if (this._device && !this._ble) {
        try {
          this._device.removeEventListener('gattserverdisconnected', this._onDisconnect);
        } catch (e) {
          log$2.error("Error removing disconnect listener: ".concat(e));
        }
        this._device = null;
      }
      if (this._ble) {
        try {
          this._ble.removeDisconnectCallback(this._onDisconnect);
          this._ble = null;
        } catch (e) {
          log$2.error("Error removing BLE disconnect callback: ".concat(e));
        }
      }
    ...
    value: function _onMessage(message) {
      var data = this._ble ? new Uint8Array(message) : new Uint8Array(message.target.value.buffer);
    ...
    value: function send(message) {
      ...
      if (this._ble) {
        return this._ble.writeCharacteristic({
          serviceId: ROOT_SERVICE_UUID,
...
var RootExtension = /*#__PURE__*/function () {
  ...
  /**
   * Setup translations for the extension
   */
  return _createClass$1(RootExtension, [{
    ...
    value: function _setupTranslations() {
      ...
      for (var locale in locales) {
        ...
        for (var key in translations) {
          formatMessage$1.setup().addTranslation(key, translations[key], locale);
        }
      }
    }
    ...
    value: function connectDevice() {
      ...
      return this.device.scan().then(function () {
        ...
        log$2.warn("Error connecting to Root rt0: ".concat(err.message));
        return Promise.resolve();
      });
    }
    ...
    value: function setMotors(args) {
      var left = Cast$1.toNumber(args.LEFT);
      var right = Cast$1.toNumber(args.RIGHT);
    ...
    value: function setLED(args) {
      var r = Cast$1.toNumber(args.RED);
      var g = Cast$1.toNumber(args.GREEN);
      var b = Cast$1.toNumber(args.BLUE);
    ...
    value: function playNote(args) {
      var freq = Cast$1.toNumber(args.FREQ);
      var dur = Cast$1.toNumber(args.DUR);
    ...
    /**
     * Called by the runtime when the extension is unloaded
     */
  }, {
    key: "dispose",
    ...
    /**
     * Called by the runtime when the extension is unloaded (for backwards compatibility)
     */
  }, {
    key: "onUninit",
...
}(); // Define block categories for Xcratch
