...
var ble = navigator.bluetooth ? WebBLE : BLE$1;
...
var RootDevice = /*#__PURE__*/function () {
  ...
  return _createClass$1(RootDevice, [{
        ...
        return _regeneratorRuntime.wrap(function _callee$(_context) {
          while (1) switch (_context.prev = _context.next) {
            case 0:
              ...
              return _context.abrupt("return", this._ble.isAvailable());
            case 4:
    ...
    value: function scan() {
      ...
      if (this._scratchLinkSupported) {
        return this._scanWithScratchLink();
      } else if (navigator.bluetooth) {
        return this._scanWithWebBluetooth();
      }
      return Promise.reject(new Error('No method available to scan for devices'));
    }

    /**
     * Scan for devices using Scratch Link
     * @returns {Promise} - resolves when a device is found and connected
     */
  }, {
    key: "_scanWithScratchLink",
    value: function _scanWithScratchLink() {
      var _this = this;
      return this._ble.requestDevice({
        ...
        _this._device = device;
        return _this._ble.connectDevice(device);
      }).then(function () {
        _this._deviceState = DEVICE_STATE.CONNECTED;
        _this._setupOnDisconnect();
        return _this._discoverCharacteristics();
      }).catch(function (err) {
        _this._deviceState = DEVICE_STATE.DISCONNECTED;
        log.warn("Error scanning with Scratch Link: ".concat(err.message));
        return Promise.reject(err);
      });
    }

    /**
     * Scan for devices using WebBluetooth
     * @returns {Promise} - resolves when a device is found and connected
     */
  }, {
    key: "_scanWithWebBluetooth",
    value: function _scanWithWebBluetooth() {
      var _this2 = this;
      return navigator.bluetooth.requestDevice({
        ...
        _this2._device = device;
        device.addEventListener('gattserverdisconnected', _this2._onDisconnect);
        ...
        _this2._characteristics.tx = characteristics[0];
        _this2._characteristics.rx = characteristics[1];
        _this2._deviceState = DEVICE_STATE.CONNECTED;

        // Start listening for sensor data
        return _this2._characteristics.tx.startNotifications().then(function () {
          _this2._characteristics.tx.addEventListener('characteristicvaluechanged', _this2._onMessage);
        ...
        log.warn("Error scanning with WebBluetooth: ".concat(err.message));
        return Promise.reject(err);
      });
    }
    ...
    value: function _discoverCharacteristics() {
      var _this3 = this;
      return this._ble.startNotifications(ROOT_SERVICE_UUID, ROOT_UART_TX_CHARACTERISTIC_UUID, this._onMessage).then(function () {
        // Request initial sensor values
        _this3.getSensors();
      });
    }
    ...
    value: function _onDisconnect() {
      this._deviceState = DEVICE_STATE.DISCONNECTED;
      if (this._device && !this._scratchLinkSupported) {
        try {
    ...
    value: function _setupOnDisconnect() {
      if (this._scratchLinkSupported && this._ble) {
    ...
    value: function _onMessage(message) {
      var data = this._scratchLinkSupported ? new Uint8Array(message) : new Uint8Array(message.target.value.buffer);
    ...
    value: function send(message) {
      ...
      if (this._scratchLinkSupported) {
        return this._ble.write(ROOT_SERVICE_UUID, ROOT_UART_RX_CHARACTERISTIC_UUID, message.buffer);
      } else {
